...
func New(hostKeyPath string, domain string, maxTunnelsPerIP int, maxTotalTunnels int, maxConnectionsPerMinute int) (*Server, error) {
	...
	s := &Server{
		...
		abuseTracker:            NewAbuseTracker(maxConnectionsPerMinute),
		domain:                  domain,
		maxTunnelsPerIP:         maxTunnelsPerIP,
		maxTotalTunnels:         maxTotalTunnels,
...
func (s *Server) CheckAndReserveConnection(clientIP string) error {
	...
	if !s.abuseTracker.CheckConnectionRate(clientIP) {
		return fmt.Errorf("connection rate limit exceeded: max %d connections per minute. Repeated violations will result in a temporary block", s.maxConnectionsPerMinute)
	}
