...
func isOLItem(line string) bool {
	return compiledRe(`^\d+\.\s+`).MatchString(line)
}

// RenderPage wraps a rendered markdown body in a full HTML page with optional sidebar nav.
func RenderPage(title, body, nav string) string {
...
func Handler(root string) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if statErr == nil && !info.IsDir() &&
			...
			page := RenderPage(title, body, nav)
