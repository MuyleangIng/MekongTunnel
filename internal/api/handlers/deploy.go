...
func (h *DeployHandler) Upload(w http.ResponseWriter, r *http.Request) {
	...
	if deployType != "static" && deployType != "nextjs" && deployType != "php" {
		response.BadRequest(w, "invalid type — must be static, nextjs, or php")
		return
	}
	...
	if active >= deployMaxPerUser {
		if err := h.stopUserDeployments(ctx, claims.UserID); err != nil {
			response.InternalError(w, err)
			return
		}
	}
...
func (h *DeployHandler) listDeployments(ctx context.Context, userID string) ([]deployRecord, error) {
	rows, err := h.DB.Pool.Query(ctx,
		`SELECT id, user_id, subdomain, domain, type, status, files_path, size_bytes, created_at, expires_at, stopped_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.UserID, &r.Subdomain, &r.Domain, &r.Type,
			&r.Status, &r.FilesPath, &r.SizeBytes, &r.CreatedAt, &r.ExpiresAt, &r.StoppedAt); err != nil {
			return nil, err
		}
		recs = append(recs, r)
	}
...
func (h *DeployHandler) getDeployment(ctx context.Context, subdomain string) (*deployRecord, error) {
	...
	err := h.DB.Pool.QueryRow(ctx,
		...
		&r.Status, &r.FilesPath, &r.SizeBytes, &r.CreatedAt, &r.ExpiresAt, &r.StoppedAt)
	if err != nil {
		return nil, err
	}
	return &r, nil
