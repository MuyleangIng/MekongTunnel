...
func printMainHelp() {
	...
	fmt.Fprintln(os.Stderr, "Usage:  mekong [OPTIONS] <port>")
	fmt.Fprintln(os.Stderr, "        mekong <command> [args]")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Expose a local port to the internet over SSH.")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Common Commands:")
	fmt.Fprintln(os.Stderr, "  <port>      Expose localhost:<port> with a random URL  (e.g. mekong 3000)")
	fmt.Fprintln(os.Stderr, "  ps          List active tunnels (table view)")
	fmt.Fprintln(os.Stderr, "  status      List active tunnels (detailed view)")
	fmt.Fprintln(os.Stderr, "  stop        Stop background tunnel(s)                  (--all to stop all)")
	fmt.Fprintln(os.Stderr, "  logs        Show or follow daemon logs                 (-f to follow, [<port>|<name>] to filter)")
	...
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Management Commands:")
	fmt.Fprintln(os.Stderr, "  subdomain   Manage reserved subdomains                 (alias: sd)")
	fmt.Fprintln(os.Stderr, "  domain      Manage custom domains                      (alias: dm)")
	fmt.Fprintln(os.Stderr, "  login       Authenticate to MekongTunnel")
	fmt.Fprintln(os.Stderr, "  logout      Remove saved credentials")
	fmt.Fprintln(os.Stderr, "  whoami      Show current account info")
	fmt.Fprintln(os.Stderr, "")
	...
	fmt.Fprintln(os.Stderr, "  subdomain <name>             Reserve a new subdomain")
	fmt.Fprintln(os.Stderr, "  subdomain delete <name>      Delete a reserved subdomain  (--yes to skip confirm)")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Domain Commands:")
	fmt.Fprintln(os.Stderr, "  domain                           List custom domains")
	fmt.Fprintln(os.Stderr, "  domain add <d>                   Add a custom domain")
	fmt.Fprintln(os.Stderr, "  domain connect <d> <sub>         Add + verify + target in one step")
	fmt.Fprintln(os.Stderr, "  domain verify <d>                Run a DNS/HTTPS verification check")
	fmt.Fprintln(os.Stderr, "  domain wait <d>                  Poll until DNS and HTTPS are ready")
	fmt.Fprintln(os.Stderr, "  domain target <d> <sub>          Point domain to a reserved subdomain")
	fmt.Fprintln(os.Stderr, "  domain delete <d>                Remove a custom domain  (--yes to skip confirm)")
	fmt.Fprintln(os.Stderr, "  doctor <d>                       Check custom-domain DNS and HTTPS")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Global Options:")
	fmt.Fprintln(os.Stderr, "  -p, --port <port>           Local port to expose")
	fmt.Fprintln(os.Stderr, "  -d                          Run in background (daemon mode)")
	fmt.Fprintln(os.Stderr, "  -e, --expire <dur>          Tunnel lifetime  (1d, 1w, 1mo — default 1w with -sd)")
	fmt.Fprintln(os.Stderr, "  -sd, --subdomain <name>     Use a reserved subdomain  (default 1w, max 1mo)")
	fmt.Fprintln(os.Stderr, "      --token <tok>           Auth token  (flag > MEKONG_TOKEN > login)")
	fmt.Fprintln(os.Stderr, "      --upstream-host <host>  Override Host header for vhost apps")
	fmt.Fprintln(os.Stderr, "      --skip-browser-warning  Skip the browser warning page for this tunnel")
	fmt.Fprintln(os.Stderr, "      --no-warning            Alias for --skip-browser-warning")
	fmt.Fprintln(os.Stderr, "      --no-qr                 Disable QR code")
	fmt.Fprintln(os.Stderr, "      --no-clipboard          Disable clipboard copy")
	fmt.Fprintln(os.Stderr, "      --no-reconnect          Disable auto-reconnect")
	fmt.Fprintln(os.Stderr, "      --server <host>         Server hostname")
	fmt.Fprintln(os.Stderr, "      --ssh-port <port>       SSH port  (default 22)")
...
func printDeployHelp() {
	...
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "  Deploy:")
	fmt.Fprintln(os.Stderr, "    mekong deploy ./dist             static HTML/CSS/JS")
	fmt.Fprintln(os.Stderr, "    mekong deploy ./docs             markdown docs site")
	fmt.Fprintln(os.Stderr, "    mekong deploy ./.next            Next.js build")
	fmt.Fprintln(os.Stderr, "    mekong deploy ./src              PHP project")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "  Manage:")
	fmt.Fprintln(os.Stderr, "    mekong deploy list               list all deployments")
	fmt.Fprintln(os.Stderr, "    mekong deploy -a                 list all (shorthand)")
	fmt.Fprintln(os.Stderr, "    mekong deploy log <subdomain>    view deploy logs")
	fmt.Fprintln(os.Stderr, "    mekong deploy info <subdomain>   deployment details")
	fmt.Fprintln(os.Stderr, "    mekong deploy open <subdomain>   open in browser")
	fmt.Fprintln(os.Stderr, "    mekong deploy quota              storage usage")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "  Stop / Delete:")
	fmt.Fprintln(os.Stderr, "    mekong deploy stop <subdomain>   stop a deployment")
	fmt.Fprintln(os.Stderr, "    mekong deploy stop --all         stop all deployments")
	fmt.Fprintln(os.Stderr, "    mekong deploy delete <subdomain> hard delete (removes files)")
	fmt.Fprintln(os.Stderr, "    mekong deploy delete --all       delete all deployments")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "  Redeploy:")
	fmt.Fprintln(os.Stderr, "    mekong deploy redeploy <sub> <path>")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "  Expiry (--expire / -e):")
	fmt.Fprintln(os.Stderr, "    --expire 1m   1mo   1 month")
	fmt.Fprintln(os.Stderr, "    --expire 1w   2w    1 or 2 weeks")
	fmt.Fprintln(os.Stderr, "    --expire 1d   7d    days")
	fmt.Fprintln(os.Stderr, "    --expire 48h  48    hours (bare number = hours)")
	fmt.Fprintln(os.Stderr, "    --expire 30m        30 minutes")
	fmt.Fprintln(os.Stderr, "")
